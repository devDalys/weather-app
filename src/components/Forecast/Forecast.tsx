      ...
      {forecast.slice(0, 5).map((cast, id) => {
        return (
          <div className="forecast__item" key={id}>
            <span className="forecast__item_date">
              {getForecastDays(cast.date)}
            </span>
            <span className="forecast__item_day">{t(cast.day)}</span>
            <span className="forecast__item_day">
              {getForecastIcon(cast.code)}
            </span>
            <span className="forecast__item_cast">
